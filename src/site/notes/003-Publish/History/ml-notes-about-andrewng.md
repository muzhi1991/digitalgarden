...
![5eb364cc5732428c695e2aa90138b01b.png](https://fodi.389266.xyz/images/IMG-f2c9efab604a4d3f.webp)
...
- 特征缩放：我们要保证这些特征都具有相近的尺度，这将帮助梯度下降算法更快地**收敛**。最简单的方法是令：$x_{n}=\frac{x_{n}-\mu_{n}}{s_{n}}$，其中 ${\mu_{n}}$是平均值，${s_{n}}$是标准差。
  ![](https://fodi.389266.xyz/images/IMG-324d4bea4d1ec87c.webp)
...
![6590923ac94130a979a8ca1d911b68a3.png](https://fodi.389266.xyz/images/IMG-994279786de2749f.webp)

上述模型的表示可以理解为决策边界：
![f71fb6102e1ceb616314499a027336dc.jpg](https://fodi.389266.xyz/images/IMG-695b5a40ecf50fff.webp )
...
![197d605aa74bee1556720ea248bab182.jpg](https://fodi.389266.xyz/images/IMG-02f6459ed64fec7b.webp )
...
> ![8b94e47b7630ac2b0bcb10d204513810.jpg](https://fodi.389266.xyz/images/IMG-62008adb3fc26609.webp)
...
![ffa56adcc217800d71afdc3e0df88378.jpg](https://fodi.389266.xyz/images/IMG-ff6ccc0c2d5ca9cd.webp )
...
问题如下
![54d7903564b4416305b26f6ff2e13c04.png](https://fodi.389266.xyz/images/IMG-178c701b4c51aab5.webp )
...
回归问题如下：
![72f84165fbf1753cd516e65d5e91c0d3.jpg](https://fodi.389266.xyz/images/IMG-e83fe55a1b665fbf.webp )

分类问题如下：
![be39b497588499d671942cc15026e4a2.jpg](https://fodi.389266.xyz/images/IMG-8b7baa1a0d0459be.webp )
...
效果如下：
![ea76cc5394cf298f2414f230bcded0bd.jpg](https://fodi.389266.xyz/images/IMG-ff2a086a8bd868aa.webp )
...
3. 求解方法-正规方程
   ![71d723ddb5863c943fcd4e6951114ee3.png](https://fodi.389266.xyz/images/IMG-420de9b022c6b776.webp)
...
![6167ad04e696c400cb9e1b7dc1e58d8a.png](https://fodi.389266.xyz/images/IMG-5613d30443ce5ccf.webp )

### 模型

![8293711e1d23414d0a03f6878f5a2d91.jpg](https://fodi.389266.xyz/images/IMG-a26c67db4e91457a.webp )
...
矩阵表示为：
![20171101224053.png](https://fodi.389266.xyz/images/IMG-e46035caad083f58.webp)
即：$\theta \cdot X=a​$

### FORWARD PROPAGATION

每一层的计算方法如下
![2e17f58ce9a79525089a1c2e0b4c0ccc.png](https://fodi.389266.xyz/images/IMG-8cd0286ce0cbdddf.webp )
...
![2ea8f5ce4c3df931ee49cf8d987ef25d.jpg](https://fodi.389266.xyz/images/IMG-bd48575e610a4e5b.webp)
...
![432c906875baca78031bd337fe0c8682.jpg](https://fodi.389266.xyz/images/IMG-10745d10986ada19.webp )

### 多分类问题

上面的模型的输出层只有一个神经元。如果多分类，如4分类可以输出层4个神经元分别用来表示4类
![f3236b14640fa053e62c73177b3474ed.jpg](https://fodi.389266.xyz/images/IMG-cae58ba7ef323602.webp )
其中输出层的值可能如下：
![685180bf1774f7edd2b0856a8aae3498.png](https://fodi.389266.xyz/images/IMG-2a5431dfd14ac233.webp )

### cost function

我们定义神经网络如下：
![8f7c28297fc9ed297f42942018441850.jpg](https://fodi.389266.xyz/images/IMG-aa662c95ad1ecd07.webp )
...
分析下面的反向传播过程
![6a0954ad41f959d7f272e8f53d4ee2de.jpg](https://fodi.389266.xyz/images/IMG-6a62776d2ca4e08f.webp )
以一次过程过程（一个样本）为例：
...
![5514df14ebd508fd597e552fbadcf053.jpg](https://fodi.389266.xyz/images/IMG-dbebf90af59721a7.webp )
...
3. 使用60%的数据作为训练集，使用 20%的数据作为交叉验证集，使用20%的数据作为测试集
   ![7cf1cd9c123a72ca4137ca515871689d.png](https://fodi.389266.xyz/images/IMG-01f9f53c9dfb23c0.webp )
...
下面的图就可以理解偏差方差：
![20c6b0ba8375ca496b7557def6c00324.jpg](https://fodi.389266.xyz/images/IMG-63f24bfa4c9c9938.webp )
...
1. 如何判断：将训练集代价函数$J_{train}$和交叉验证集的代价函数$J_{cv}$误差与多项式的次数绘制在同一张图表上来帮助分析（注意横轴是模型复杂度，从小到大）：
   ...
   - 交叉验证集误差远大于训练集误差时：方差/过拟合
     ![25597f0f88208a7e74a3ca028e971852.png](https://fodi.389266.xyz/images/IMG-9618218962f83e82.webp)
2. 当**使用正则化来代控制模型复杂度**时，如何选择，绘制$J_{train}$与$J_{cv}$图（注意横轴是$\lambda$ ,越大，模型复杂度月低，与上图相反）
   - 当 $\lambda$ 较小时，训练集误差较小（过拟合）而交叉验证集误差较大
   - 随着 $\lambda$ 的增加，训练集误差不断增加（欠拟合），而交叉验证集误差则是先减小后增加
     ![38eed7de718f44f6bb23727c5a88bf5d.png](https://fodi.389266.xyz/images/IMG-eb89bd590bedcd5a.webp)
3. 另一种判断方法是使用**学习曲线（Learning Curves）**，他是一种快速的合理性检验方法（sanity check）。他也是绘制绘制$J_{train}$与$J_{cv}$图，但是横轴是数据训练集大小m。思想是：当训练较少行数据的时候，训练的模型将能够非常完美地适应较少的训练数据，但是训练出来的模型却不能很好地适应交叉验证集数据或测试集数据
   - 识别高偏差/欠拟合：作为例子，我们尝试用一条直线来适应下面的数据，可以看出，无论训练集有多么大，误差都不会有太大改观（竖轴值都很大，且两者接近），此时再收集样本没什么用。
     ![4a5099b9f4b6aac5785cb0ad05289335.jpg](https://fodi.389266.xyz/images/IMG-b872654086213cf3.webp)
   - 识别高方差/过拟合：假设我们使用一个非常高次的多项式模型，并且正则化非常小，可以看出，当交叉验证集误差远大于训练集误差时（竖轴值都小，前两者分开），往训练集增加更多数据可以提高模型的效果
     ![2977243994d8d28d5ff300680988ec34.jpg](https://fodi.389266.xyz/images/IMG-5fbc08d8afaf6072.webp)
...
- trade-off: 一种方法是计算**F1 值**（**F1 Score**），${F}_{1}Score:2\frac{PR}{P+R}$
  ![84067e23f2ab0423679379afc6ed6caf.png](https://fodi.389266.xyz/images/IMG-761810a583305e35.webp)
...
![befe860fd4b1aef2f6eebf617baf5877.jpg](https://fodi.389266.xyz/images/IMG-4af3fffbd41466d1.webp )
...
![5a63e35db410fdb57c76de97ea888278.png](https://fodi.389266.xyz/images/IMG-ad2f76ec39b9be75.webp )

理解：**从逻辑回归演变而来**
![59541ab1fda4f92d6f1b508c8e29ab1c.png](https://fodi.389266.xyz/images/IMG-deb63e6b7034497b.webp )
注意点：
...
1. 当我们把$C$设置成了非常大的常数，我们将会很希望找到一个使第一项为0的最优解。因此，如果 $y^{(i)}$是等于1，那么需要$\theta^Tx^{(i)}>=1$， 如果 $y^{(i)}$是等于0，那么需要$\theta^Tx^{(i)}<=-1$。这两个约束条件总结如下：
   ![](https://fodi.389266.xyz/images/IMG-5b1b046560f2ea9d.webp)
2. 决策边界的效果如下，黑色的线优于绿色，他具有最大的间隔：
   ![e68e6ca3275f433330a7981971eb4f16.png](https://fodi.389266.xyz/images/IMG-87b5b469696a1ad8.webp)
3. 理解为何上述约束会生成黑色的线
   ...
   - 下图中左边的投影（红色）比较小（p小），为了>=1则$\theta$则比较大，对比右图，会发现他的$\theta$比较小，**对应的图中间隔比较大**
     ![](https://fodi.389266.xyz/images/IMG-706c12ca946edc36.webp)
...
4. 映射的方法：对每个样本计算m个新特征：$f_{1}=\operatorname{similarity}\left(x, l^{(1)}\right)=e\left(-\frac{\left\|x-l^{(1)}\right\|^{2}}{2 \sigma^{2}}\right)$，分别计算$f_1,f_2...f_m$。这里的$l^{(1)}$是样本集合中的一个其他样本，叫做**地标**，$f_i$刻画了与$l^{(i)}$的相似距离。这里的similarity就是核函数，这个叫高斯核
   ![](https://fodi.389266.xyz/images/IMG-5b6477b04c276cdf.webp)
...
1. 为什么这种映射是可行的？感性的理解一下，样本空间如下，当样本处于洋红色的点位置处，因为其离$l^{(1)}$更近，但是离$l^{(2)}$和$l^{(3)}$较远，因此$f_1$接近1，而$f_2$,$f_3$接近0。因此$h_θ(x)=θ_0+θ_1f_1+θ_2f_2+θ_1f_3>0$，因此预测$y=1$。同理可以求出，对于离$l^{(2)}$较近的绿色点，也预测$y=1$，但是对于蓝绿色的点，因为其离三个地标都较远，预测$y=0$。
   ![](https://fodi.389266.xyz/images/IMG-67e1a909d6afea8c.webp)
2. 高斯核的作用？高斯核的图像如下，可以发现：
   ...
   ![](https://fodi.389266.xyz/images/IMG-1c0df5f0d241618d.webp)
...
![](https://fodi.389266.xyz/images/IMG-80c6b6a0b589946a.webp)
...
![](https://fodi.389266.xyz/images/IMG-2b523b7ca260a73f.webp)
...
![](https://fodi.389266.xyz/images/IMG-579d9cb3b9355505.webp)

> 对比线性回归目标不同，方法相似，线性回归目标是预测，这里没有.
> 投射的方向不同，如下所示，左边的是线性回归的误差（垂直于横轴投影），右边则是主要成分分析的误差（垂直于红线投影）
> ![](https://fodi.389266.xyz/images/IMG-49135e58bfcc4717.webp)
...
3. 第三步是计算协方差矩阵$Σ$的**特征向量**（**eigenvectors**）:在Octave中用svd方法
   ![0918b38594709705723ed34bb74928ba.png](https://fodi.389266.xyz/images/IMG-638052a83cb954f8.webp )
...
![65afdea865d50cba12d4f7674d599de5.png](https://fodi.389266.xyz/images/IMG-9779eff52c8e1cee.webp )
模型定义：
...
![ba47767a11ba39a23898b9f1a5a57cc5.png](https://fodi.389266.xyz/images/IMG-d3b56eabd3d72dfe.webp )
$p(x)$的图像如下
![82b90f56570c05966da116c3afe6fc91.jpg](https://fodi.389266.xyz/images/IMG-a5ff81a945f0eeaf.webp )
...
![](https://fodi.389266.xyz/images/IMG-6b5ae5406d591fc8.webp)
...
2. 不独立的特征：我们经常发现模型一些异常的数据可能也会有较高的$p(x)$值，这个可能是特征有了问题，一般会使用**误差分析**的方法，发现一些新的特征--通常可以通过**将一些相关特征进行组合**，来获得一些新的更好的特征。（简而言之把不独立的特征组合形成一个特征消除了不独立的问题）
   ![](https://fodi.389266.xyz/images/IMG-d28a13de11628914.webp)

### 多元高斯分布

解决特征相关的问题一个更『高级』的方法是使用多元高斯分布，他的分布如下图蓝色所示：是**一个歪的椭圆**
![](https://fodi.389266.xyz/images/IMG-6f6f6b4ace2090da.webp)
...
![](https://fodi.389266.xyz/images/IMG-5fbcac7121451a73.webp)
上图是5个不同的模型，从左往右依次分析：
...
多元高斯分布应用过程如下：
![](https://fodi.389266.xyz/images/IMG-19c13a482570721c.webp)
...
![](https://fodi.389266.xyz/images/IMG-41fd7dbc243e82f9.webp)
...
![](https://fodi.389266.xyz/images/IMG-818e7db040855516.webp)
...
问题：识别图片中包含的所有文字
![095e4712376c26ff7ffa260125760140.jpg](https://fodi.389266.xyz/images/IMG-c3161d4d3828cdf8.webp )
...
3. 字符分类（**Character classification**）——确定每一个字符是什么
   ...
   ![](https://fodi.389266.xyz/images/IMG-8d5cec6513b915a8.webp)
...
> ![](https://fodi.389266.xyz/images/IMG-e854f77a25101ba2.webp)
...
效果如下（绿色为最终有效区域）
![](https://fodi.389266.xyz/images/IMG-7152eb72eac0c576.webp)
...
![](https://fodi.389266.xyz/images/IMG-51db085acc6f21a9.webp)
![](https://fodi.389266.xyz/images/IMG-1203103c8c315db2.webp)
...
![](https://fodi.389266.xyz/images/IMG-c9715ecf60b6a5a3.webp)
