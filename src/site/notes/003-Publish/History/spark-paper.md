...
3. 数据集在spark中表示成**一串scala的object**：他们表示了** the lineage of each RDD**，且每个对象包括
  * 一个指向他parent的指针
  * parent如何转换为这个RDD
    ![](https://fodi.389266.xyz/images/IMG-2ff114dec10c672f.webp)
...
* 为了错误容忍，RDD提供了一种『受限的共享内存』：他是基于粗粒度的transformation，而不是细粒度的对共享状态的update。
  ...
  * 对比RDD：logging the transformations used to build a dataset (itslineage) rather than the actual data
  ![](https://fodi.389266.xyz/images/IMG-8f00d8bf12af142b.webp)
* rdd的特征
  * 错误容忍（最重要的，也是最大的挑战）
  * 并行的数据结构
  * 可以缓存**用户指定**的中间数据
...
* spark架构，用户写driver，连接n个worker
  ...
  * worker是**常驻进程**，可以在内存中存储RDD的各个分区
  ![](https://fodi.389266.xyz/images/IMG-309071e038e5c1fe.webp)
...
#### RDD的操作（operation）
![](https://fodi.389266.xyz/images/IMG-1c023560a2459db4.webp)
注意点：
...
* 例子1：**PageRank（重点）**
  * 理解算法，同时看一下RDD的图谱。
  ![|337x250](https://fodi.389266.xyz/images/IMG-7f74375bf83b56ee.webp)
  * 几个优化要点（**对于多次迭代的应用通用**）
...
![](https://fodi.389266.xyz/images/IMG-b490de8d5c178dcd.webp)
...
![](https://fodi.389266.xyz/images/IMG-cbea29609a28908e.webp)
...
* 当运行action时，scheduler检测RDD的lineage，生成一个stage的DAG图
  ...
  * 任何已经计算完成的分区（如cached过的）可以短路parent RDD的计算、
  ![](https://fodi.389266.xyz/images/IMG-0e33eb5b159fcf27.webp)
...
![](https://fodi.389266.xyz/images/IMG-365156160b216939.webp)
...
![](https://fodi.389266.xyz/images/IMG-c98adda8373baeca.webp)
